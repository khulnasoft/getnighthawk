...
Nighthawk is a versatile HTTP load testing tool built out of a need to drill HTTP services with a constant request rate or with an adaptive request rate. Khulnasoft offers a custom distribution of Nighthawk with intelligent adaptive load controllers to automatically identify optimatal configurations for your service mesh deployment. As a KhulnaSoft performance characterization tool supporting HTTP/HTTPS/HTTP2, Nighthawk is Meshplay's (and Envoy's) load generator and is written in C++.
...
<img src="/assets/images/screenshots/Nighthawk+Meshplay.svg" style="width:90%;padding:4%;align:center" />
